...
export default function NotificationsPage() {
  ...
  const alerts = [
    { id: 1, title: 'Welcome!', body: 'Start exploring the catalog to find items.', time: 'Just now' },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FDF8F7] dark:bg-[#1A0B2E] pb-32">
      <header className="p-6 flex items-center gap-4">
        ...
        <h1 className="text-xl font-black dark:text-[#FFF9E3]">Notifications</h1>
      </header>

      <main className="px-6 space-y-4">
        {alerts.map(a => (
          <div key={a.id} className="bg-white dark:bg-[#2D1B4E] p-5 rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] flex gap-4">
            <div className="w-10 h-10 bg-[#F0F7F6] dark:bg-[#1A0B2E] rounded-2xl flex items-center justify-center text-[#7ED7C1]">
              <Bell size={20} />
            </div>
            <div className="flex-1">
              <h3 className="text-sm font-black dark:text-[#E0D7FF]">{a.title}</h3>
