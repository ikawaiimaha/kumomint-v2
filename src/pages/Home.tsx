  ...
  return (
    <Layout title="" showNav={true}>
      <div className="space-y-6">
        {/* HERO */}
        <section className="bg-white/80 border border-[rgba(165,214,200,0.18)] rounded-[32px] p-6 shadow-sm relative overflow-hidden">
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="text-2xl font-bold text-gray-800 truncate">Welcome, {welcomeName}</h2>
            </div>
          </div>
          {!userId && (
            <div className="mt-4 flex gap-2">
              <button onClick={() => navigate('/login')} className="bg-[#A5D6C8] px-6 py-2 rounded-xl font-bold text-[#2E2A28]">Sign In</button>
              <button onClick={() => navigate('/register')} className="border border-[rgba(165,214,200,0.3)] px-6 py-2 rounded-xl font-bold text-[#2E2A28]">Join</button>
            </div>
          )}
        </section>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white p-4 rounded-2xl border border-[rgba(165,214,200,0.1)] text-center shadow-sm">
            <Cloud className="mx-auto mb-1 text-blue-300" size={20} />
            <p className="text-xl font-bold text-[#2E2A28]">{profile?.dream_mints || 0}</p>
            <p className="text-[10px] text-gray-400">MINTS</p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-[rgba(165,214,200,0.1)] text-center shadow-sm">
            <Zap className="mx-auto mb-1 text-yellow-400" size={20} />
            <p className="text-xl font-bold text-[#2E2A28]">{profile?.login_streak || 0}</p>
            <p className="text-[10px] text-gray-400">STREAK</p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-[rgba(165,214,200,0.1)] text-center shadow-sm">
            <Heart className="mx-auto mb-1 text-pink-300" size={20} />
            <p className="text-xl font-bold text-[#2E2A28]">{matches.length}</p>
        ...
        <section className="space-y-3">
          <div className="flex justify-between items-center">
            <h3 className="font-bold text-lg text-[#2E2A28]">Trending Items</h3>
            <button onClick={() => navigate('/catalog')} className="text-sm text-[#4E927E] font-semibold">See All</button>
          </div>
          
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 -mx-4 px-4 scroll-smooth">
            {trending.map((item) => (
              <div key={item.id} className="w-36 shrink-0 bg-white rounded-3xl border border-[rgba(165,214,200,0.14)] overflow-hidden shadow-[0_10px_24px_rgba(46,42,40,0.04)] transition-transform hover:-translate-y-1">
                ...
                <div className="p-3">
                  <p className="text-xs font-bold text-[#2E2A28] truncate">{item.name}</p>
