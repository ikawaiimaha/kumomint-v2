...
export default function ProfilePage() {
  ...
  if (!user) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-[#FDF8F7] dark:bg-[#1A0B2E]">
      <Package size={48} className="text-gray-200 dark:text-[#2D1B4E] mb-4" />
      <h2 className="font-black text-xl mb-6 text-[#2E2A28] dark:text-[#FFF9E3]">Login Required</h2>
      ...
      <main className="px-5 space-y-6">
        ...
        <div className="bg-white dark:bg-[#2D1B4E] rounded-[40px] p-6 shadow-sm border border-[#F0E6E4] dark:border-[#483475] relative">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            >
              {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" alt="" /> : <span className="text-3xl font-black text-gray-300 dark:text-[#483475]">{username?.charAt(0).toUpperCase() || 'K'}</span>}
          ...
          {isEditing ? (
            <div className="space-y-4">
              <textarea 
                className="w-full bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-2xl p-4 text-xs font-bold min-h-[80px] text-gray-500 dark:text-[#E0D7FF] border-none" 
                value={bio} 
            ...
            <>
              ...
              <button onClick={handleLogout} className="w-full flex items-center justify-between pt-4 border-t border-[#F8F9FB] dark:border-[#483475] text-red-300 active:scale-95 transition-transform">
