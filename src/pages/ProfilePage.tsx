  ...
  return (
    <div className="min-h-screen pb-32 transition-colors bg-[#FDF8F7] dark:bg-[#1A0B2E]">
      ...
      <main className="px-5 space-y-6">
        <div className="bg-white dark:bg-[#2D1B4E] rounded-[40px] p-6 shadow-sm border border-[#F0E6E4] dark:border-[#483475] relative text-center">
          ...
          {isEditing ? (
            <div className="space-y-4">
              <textarea className="w-full bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-2xl p-4 text-xs font-bold dark:text-[#FFF9E3]" value={bio} onChange={e => setBio(e.target.value)} />
              <div className="bg-[#FEF9C3] dark:bg-[#483475] p-4 rounded-2xl flex gap-3"><Info size={18} /><p className="text-[10px] font-bold text-left text-yellow-800 dark:text-[#FFF9E3]">Official HKDV avatars only.</p></div>
              <button onClick={async () => { await supabase.from('traders').update({ username, bio }).eq('id', user?.id); setIsEditing(false); fetchProfileData(); }} className="w-full bg-[#7ED7C1] dark:bg-[#A389F4] text-white py-3 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2"><Check size={16} /> {isUploading ? 'SAVING...' : 'SAVE CHANGES'}</button>
            </div>
          ) : (
