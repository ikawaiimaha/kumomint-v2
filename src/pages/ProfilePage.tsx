      ...
      <main className="px-5 space-y-6">
        <div className="bg-white dark:bg-[#2D1B4E] rounded-[40px] p-6 shadow-sm border border-[#F0E6E4] dark:border-[#483475] relative">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 text-left">
              {isEditing ? <input className="w-full bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-xl px-3 py-2 text-sm font-bold dark:text-[#FFF9E3] border-none" value={username} onChange={e => setUsername(e.target.value)} /> : 
              <><div className="flex items-center gap-2 text-left"><h2 className="text-2xl font-black dark:text-[#FFF9E3]">{profile?.username || 'kawaii'}</h2>{profile?.avatar_status === 'pending' && <Clock size={14} className="text-yellow-500" />}</div><p className="text-xs font-bold text-gray-300 dark:text-[#A389F4]">@{profile?.username}</p></>}
            </div>
          </div>
          {isEditing ? (
            <div className="space-y-4">
              <textarea className="w-full bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-2xl p-4 text-xs font-bold dark:text-[#FFF9E3] min-h-[80px] border-none" value={bio} onChange={e => setBio(e.target.value)} />
              <div className="bg-[#FEF9C3] dark:bg-[#1A0B2E] p-4 rounded-2xl flex gap-3"><Info size={18} className="text-yellow-600 shrink-0" /><p className="text-[10px] font-bold text-yellow-800 dark:text-[#FFF9E3]">Official HKDV avatars only. Manual approval required.</p></div>
              <button onClick={async () => { await supabase.from('traders').update({ username, bio }).eq('id', user?.id); setIsEditing(false); fetchProfileData(); }} className="w-full bg-[#7ED7C1] dark:bg-[#A389F4] text-white py-3 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2"><Check size={16} /> {isUploading ? 'SAVING...' : 'SAVE'}</button>
            </div>
          ) : (
            <><p className="text-xs font-bold text-gray-400 dark:text-[#A389F4] mb-4 text-left">{profile?.bio || "No bio yet."}</p><button onClick={async () => { await signOut(); navigate('/login'); }} className="w-full pt-4 border-t border-[#F8F9FB] dark:border-[#483475] text-red-300 flex items-center gap-2"><LogOut size={18} /><span className="text-sm font-bold">Sign Out</span></button></>
          )}
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white dark:bg-[#2D1B4E] p-5 rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] text-center"><p className="text-[10px] font-bold text-gray-400 dark:text-[#A389F4] uppercase">Trades</p><p className="text-2xl font-black dark:text-[#FFF9E3]">0</p></div>
          <div className="bg-white dark:bg-[#2D1B4E] p-5 rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] text-center"><p className="text-[10px] font-bold text-gray-400 dark:text-[#A389F4] uppercase">Mints</p><p className="text-2xl font-black dark:text-[#FFF9E3]">10</p></div>
        </div>
        ...
        <section>
          ...
          <div className="bg-white dark:bg-[#2D1B4E] rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] p-6 flex flex-col items-center opacity-30">
            {/* THIS USES THE PACKAGE ICON SO THE BUILD PASSES */}
