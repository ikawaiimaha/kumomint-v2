...
export default function ProfilePage() {
  ...
  if (!user) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#FDF8F7] dark:bg-[#1A0B2E] p-10">
    ...
    <div className="min-h-screen pb-32 transition-colors bg-[#FDF8F7] dark:bg-[#1A0B2E]">
      <div className="p-6 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button onClick={toggleTheme} className="text-gray-400 dark:text-[#A389F4] active:scale-90 transition-transform">
            {theme === 'dark' ? <Sun size={22} className="text-yellow-400" /> : <Moon size={22} />}
          </button>
          <button onClick={() => navigate('/notifications')} className="text-gray-400 dark:text-[#A389F4] relative">
            <Bell size={22} />
      ...
      <main className="px-5 space-y-6">
        <div className="bg-white dark:bg-[#2D1B4E] rounded-[40px] p-6 shadow-sm border border-[#F0E6E4] dark:border-[#483475] relative">
          <button onClick={() => setIsEditing(!isEditing)} className="absolute top-6 right-6 p-2 bg-gray-50 dark:bg-[#1A0B2E] rounded-full text-gray-400 active:scale-90 transition-transform">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div 
              ...
              className={`w-20 h-20 bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-[30px] flex items-center justify-center border-2 border-white dark:border-[#483475] relative overflow-hidden ${isEditing ? 'cursor-pointer' : ''}`}
            >
              {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" alt="Avatar" /> : <span className="text-3xl font-black text-gray-300 dark:text-[#483475]">{username?.charAt(0).toUpperCase() || 'K'}</span>}
              ...
              {isUploading && <div className="absolute inset-0 bg-white/50 dark:bg-[#1A0B2E]/50 flex items-center justify-center"><Sparkles className="animate-spin text-[#A389F4]" size={20} /></div>}
            </div>
          ...
          {isEditing ? (
            <div className="space-y-4">
              <textarea 
                className="w-full bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-2xl p-4 text-xs font-bold text-gray-500 dark:text-[#E0D7FF] border-none min-h-[80px]" 
                value={bio} 
              ...
              <div className="bg-[#FEF9C3] dark:bg-[#FFF9E3]/10 p-4 rounded-2xl flex gap-3 items-start border border-[#FEF08A] dark:border-[#483475]">
                ...
                <p className="text-[10px] font-bold text-yellow-800 dark:text-[#FFF9E3] leading-tight">Only HKDV avatars are allowed. Manual approval required.</p>
              </div>
              <button onClick={handleSave} className="w-full bg-[#7ED7C1] dark:bg-[#A389F4] text-white py-3 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform">
            ...
            <>
              ...
              <button onClick={async () => { await signOut(); navigate('/login'); }} className="w-full flex items-center justify-between pt-4 border-t border-[#F8F9FB] dark:border-[#483475] text-red-300 active:scale-95 transition-transform">
        ...
        {!isEditing && (
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white dark:bg-[#2D1B4E] p-5 rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] text-center">
              <p className="text-[10px] font-bold text-gray-400 dark:text-[#A389F4] uppercase">Trades</p>
              <p className="text-2xl font-black text-[#2E2A28] dark:text-[#FFF9E3]">0</p>
            </div>
            <div className="bg-white dark:bg-[#2D1B4E] p-5 rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] text-center">
              <p className="text-[10px] font-bold text-gray-400 dark:text-[#A389F4] uppercase">Mints</p>
              <p className="text-2xl font-black text-[#2E2A28] dark:text-[#FFF9E3]">10</p>
            </div>
          </div>
        )}
        ...
        {!isEditing && (
          <section>
            <div className="flex justify-between items-center mb-4 px-1">
              ...
              <button onClick={() => navigate('/wardrobe')} className="text-[10px] font-black text-[#4E927E] dark:text-[#A389F4] uppercase tracking-widest">Open</button>
            </div>
            <div className="bg-white dark:bg-[#2D1B4E] rounded-[32px] border border-[#F0E6E4] dark:border-[#483475] p-6 flex flex-col items-center opacity-30">
              <Package size={32} className="text-gray-300 dark:text-[#483475] mb-2" />
              <p className="text-[8px] font-black uppercase text-[#2E2A28] dark:text-[#A389F4]">No Items</p>
            </div>
          </section>
        )}
      </main>
