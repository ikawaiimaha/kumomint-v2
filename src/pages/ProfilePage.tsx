  ...
  return (
    <div className="min-h-screen pb-32 transition-colors bg-[#FDF8F7] dark:bg-[#1A0B2E]">
      ...
      <main className="px-5 space-y-6">
        <div className="bg-white dark:bg-[#2D1B4E] rounded-[40px] p-6 shadow-sm border border-[#F0E6E4] dark:border-[#483475] relative transition-colors">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <><div className="flex items-center gap-2"><h2 className="text-2xl font-black dark:text-[#FFF9E3]">{profile?.username}</h2>{profile?.avatar_status === 'pending' && <Clock size={14} className="text-yellow-500" />}</div><p className="text-xs font-bold text-gray-300 dark:text-[#A389F4]">@{profile?.username}</p></>}
            </div>
          </div>
          {isEditing ? (
            <div className="space-y-4">
              <textarea className="w-full bg-[#F8F9FB] dark:bg-[#1A0B2E] rounded-2xl p-4 text-xs font-bold dark:text-[#FFF9E3] min-h-[80px]" value={bio} onChange={e => setBio(e.target.value)} />
              <div className="bg-[#FEF9C3] dark:bg-[#483475] p-4 rounded-2xl flex gap-3 text-yellow-800 dark:text-[#FFF9E3]"><Info size={18} /><p className="text-[10px] font-bold">Only HKDV avatars are allowed.</p></div>
              <button onClick={async () => { await supabase.from('traders').update({ username, bio }).eq('id', user?.id); setIsEditing(false); fetchProfileData(); }} className="w-full bg-[#7ED7C1] dark:bg-[#A389F4] text-white py-3 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2">
                <Check size={16} /> {isUploading ? 'SAVING...' : 'SAVE CHANGES'}
              </button>
            </div>
          ) : (
            <><p className="text-xs font-bold text-gray-400 dark:text-[#A389F4] mb-4">{profile?.bio || "No bio yet."}</p><button onClick={async () => { await signOut(); navigate('/login'); }} className="w-full pt-4 border-t border-[#F8F9FB] dark:border-[#483475] text-red-300 flex items-center gap-2 transition-transform active:scale-95"><LogOut size={18} /><span className="text-sm font-bold">Sign Out</span></button></>
          )}
        </div>
